...
# D1.5: the MCP client glue (connection to the FIDAA knowledge server)
# lives in mcp_client.py; the transcript export builder in export_html.py.
import mcp_client
...
async def _init_session():
    ...
    mcp_client.register_for_session()


# ---------------------------------------------------------------------------
# App state (populated at startup; read-only after that)
# ---------------------------------------------------------------------------
...
# Set when startup failed (DB, MCP, models) so on_chat_start can surface it
# to users; the GUI still starts so they get a visible error.
...
async def startup():
    ...
    """
    global _startup_error, _available_models, _system_prompt
...
async def _mcp_tool_step(tc: dict) -> str:
    ...
    # (client, sentinel).
    entry = cl.context.session.mcp_sessions.get(mcp_client.FIDAA_MCP_NAME)
    mcp_session = entry if entry is not None else mcp_client.shared_session
    if mcp_session is None:
        return "Fehler: Wissensdatenbank-Server nicht verbunden."
    client = mcp_session.client

    async with cl.Step(name=mcp_client.MCP_STEP_NAMES.get(name, name), type="tool") as step:
        ...
        if structured and "results" in structured:
            ...
            for col in structured["collections"]:
                label = mcp_client.MCP_COLLECTION_LABELS.get(col["name"], col["name"])
                toc.append(
...
async def on_message(message: cl.Message):
    if message.command == "transcript":
        ...
        html_content = await cl.make_async(build_export_html)(thread_id)
        await cl.Message(
